...
void SceneManager::WakeUp()
{
	...
	// ウィンドウタイトルの設定
	SetWindowText("Game Development Super Mario Bros 2024");
...
void SceneManager::Run()
{
	...
	while (ProcessMessage() == D_SUCCESS)
	{
		...
		// ゲームを終了するか確認する
		if ((input->GetButtonUp(XINPUT_BUTTON_BACK)) ||
			(input->GetKeyUp(KEY_INPUT_ESCAPE))||current_scene->GetEndFlag())
		{
			break;
		}
...
SceneBase* SceneManager::CreateScene(eSceneType next_type)
{
	...
	case eSceneType::tika:
		return dynamic_cast<SceneBase*>(new Tika());
	case eSceneType::game:
		return dynamic_cast<SceneBase*>(new Game());
	default:
		return nullptr;
	}
}
