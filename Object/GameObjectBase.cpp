...
bool GameObjectBase::getinvincible()
{
		return invincible;
}
...
void GameObjectBase::Draw(const Vector2D& screen_offset) const
{
	...
	DrawCircleAA(location.x + screen_offset.x, location.y + screen_offset.y, 2.0f, 100,GetColor(255, 255, 0));
#endif
}
...
BoxCollision GameObjectBase::GetCollision() const
{
	return collision;
}
#else
...
const Vector2D GameObjectBase::GetObjectSize() 
{
	return this->collision.Object_size;
}
void GameObjectBase::wow(const Vector2D& screen_offset) const
{
	// オフセット値を基に画像の描画を行う
	Vector2D graph_location = this->location + screen_offset;
	DrawRotaGraphF(graph_location.x, graph_location.y, 1.0, 0.0, image, TRUE);
